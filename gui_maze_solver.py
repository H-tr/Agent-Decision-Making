...
class MazeGUI:
    def __init__(self, root, size):
        ...
        self.size = size
        self.cell_size = 50
        self.canvas = tk.Canvas(root, width=self.size[0] * self.cell_size, height=self.size[1] * self.cell_size)
        self.canvas.pack()
    ...
    def toggle_wall(self, event):
        ...
        if (i, j) in self.gridworld.walls:
            ...
            self.canvas.itemconfig(self.canvas.find_closest(event.x, event.y), fill="gray")
        ...
        for i in range(self.size[0]):
            for j in range(self.size[1]):
                if (i, j) not in self.gridworld.walls:
                    ...
                    self.canvas.itemconfig(self.canvas.find_closest((i + 0.5) * self.cell_size, (j + 0.5) * self.cell_size), fill=color)
